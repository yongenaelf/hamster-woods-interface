...
export const TaskItem = ({ item }: { item: Item }) => {
  ...
  if (isMobile) {
    return (
      <div className="w-full h-auto px-3 mb-[8px] py-2 bg-[#DEC49D] rounded-[12px] flex flex-col justify-center items-start gap-1 inline-flex">
        ...
        <div className="self-stretch flex justify-start items-center gap-2 inline-flex">
          ...
          <div className="w-14 h-6 relative">
            {item.isComplete ? (
              // eslint-disable-next-line jsx-a11y/alt-text, @next/next/no-img-element
              <img className="w-[48px] h-[22px]" src={require('assets/images/button-task-ok.png').default.src} />
            ) : (
              <>
  ...
  return (
    <div className="w-full h-auto px-3 py-2 bg-[#DEC49D] rounded-[12px] inline-flex justify-start items-center gap-2 mb-[8px]">
      ...
      <div className="w-14 h-6 relative">
        <div className="w-14 h-6 absolute bg-[#FEB800] shadow-[inset_0px_2px_1px_#FFE051] rounded-[30px]" />
        {/* <div className="absolute left-4 top-1 text-[#52300B] text-[16px] font-paytone-one font-normal leading-[16px] break-words">
          Go
        </div> */}
        <div className="absolute left-4 top-[3px] text-white text-[16px] font-paytone-one font-normal leading-[16px] break-words font-paytone">
          Go
